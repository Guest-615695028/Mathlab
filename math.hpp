...
namespace Mathlab {
	...
	template <Arithmetic _T, Arithmetic _U> inline constexpr Promoted<_T, _U> fmod(_T x, _U y) noexcept {
		if (y <= 0) return y ? fmod(x, -y) : nan(errno = EDOM);
		else if (x <= 0) return x ? fmod(-x, y) : 0;
		else if (!isfinite(x)) return nan(errno = EDOM);
		auto a = x >= 2 * y ? fmod(x, 2.0L * y) : x;
		return a >= y ? a - y : a <= -y ? a + y : a;
	}
	template <Arithmetic _T, Arithmetic _U> inline constexpr Promoted<_T, _U> remainder(_T x, _U y) noexcept {
		if (y <= 0) return y ? remainder(x, -y) : nan(errno = EDOM);
		else if (!isfinite(x)) return nan(errno = EDOM);
		else if (y == infinity()) return x;
		auto a = x > y || x < -y ? remainder(x, 2.0L * y) : x;
		return a > y * 0.5L ? a - y : a < -y * 0.5L ? a + y : a;
	}
	// 1.2 Integer rounding functions
	template <Arithmetic _T, Arithmetic _U = _T> inline constexpr auto trunc(_T x) noexcept {
		if constexpr (Integral<_T>) return x;
		else return x - fmod(x, 1);
	}
	template <Arithmetic _T, Arithmetic _U = _T> inline constexpr auto round(_T x) noexcept {
		if constexpr (Integral<_T>) return x;
		else return x - remainder(x, 1);
	}
	template <Arithmetic _T, Arithmetic _U = _T> inline constexpr auto ceil(_T x) noexcept {
		if constexpr (Integral<_T>) return x;
		else return trunc(x) + (x > 0 && fmod(x, 1));
	}
	template <Arithmetic _T, Arithmetic _U = _T> inline constexpr auto floor(_T x) noexcept {
		if constexpr (Integral<_T>) return x;
		else return trunc(x) - (x < 0 && fmod(x, 1));
	}
	// 1.3 Bit manipulation
	inline constexpr bool isPowOf2(uintmax_t i) {
		return i && !(i & i - 1);
	}
	template <Integral _I = uintmax_t> inline constexpr _I bitFloor(_I i) {
		if (i < 0) return limits(i).min();
	...
	template <Integral _I = uintmax_t> inline constexpr _I bitWidth(_I i) {
		if (i < 0) return i + 1 ? bitWidth(~i) : 1;
		else if (i == 0) return 0;
		_I j = sizeof(i) * 8, k = _I(-1) < 0 ? 2 : 1;
		while (j >>= 1) if (i >> j) i >>= j, k += j;
		return k;
	}
	...
	template <Arithmetic _T> inline constexpr Promoted<_T> exp(_T x) noexcept {
		if (x > 1 || x < -1) {
			if (x >= LDBL_MAX_EXP) return infinity();
			else if (x <= LDBL_MIN_EXP) return 0;
	...
	template <Arithmetic _T> inline constexpr Promoted<_T> log(_T x) noexcept {
		if (x < 0) return nan(errno = EDOM);
	...
	template <Arithmetic _T> inline constexpr Promoted<_T> log2(_T x) noexcept {
		return x < 0 ? nan(errno = EDOM) : x == 0 ? -infinity() :
			x <= 0.5 ? log2(2.0l * x) - 1 :
			x > 1 ? -log2(1.0l / x) : log(x) * M_LOG2E;
	}
	template <Arithmetic _T> inline constexpr Promoted<_T> log10(_T x) noexcept {
		return x < 0 ? nan(errno = EDOM) : x == 0 ? -infinity() :
			x <= 0.1 ? log10(10.0l * x) - 1 :
			x > 1 ? -log10(1.0l / x) : log(x) * M_LOG10E;
	}
	template <Arithmetic _T, Arithmetic _U> inline constexpr Promoted<_T, _U> logn(_T x, _U y) noexcept {
		return x < 0 || y <= 0 || y == 1 ? nan(errno = EDOM) :
			x == 0 ? y < 1 ? infinity() : -infinity() :
			x == 1 ? 0 :
			x > 1 ? -logn(1.0l / y, y) : log(x) / log(y);
	}
	template <Arithmetic _T> inline constexpr Promoted<_T> sigmoid(_T x) noexcept {
		return 1.0l / (1.0l + exp(x));
	}
	// 2.3 Power functions
	template <Arithmetic _T, Arithmetic _U> inline constexpr Promoted<_T, _U> pow(_T x, _U y) noexcept {
		if (y == 0 || x == 1) return 1;
		else if (y < 0) return pow(1 / x, -y); // y>0 vvv
		else if (y == infinity()) return abs(x) > 1 ? infinity() : x == -1;
		auto a = fmod(y, 2);
		if (signbit(x)) return a == 1 ? -pow(-x, y) : a == 0 ? pow(-x, y) : nan(errno = EDOM);
		else return exp(y * log(x));
	}
	template <Arithmetic _T> inline constexpr Promoted<_T> sqrt(_T x) noexcept {
	...
	template <Arithmetic... _T> inline constexpr Promoted<_T...> hypot(_T... t) noexcept {
		if (!(t && ...)) return 0;
		auto l = ((1.0l * t * t) + ...);
		return isinf(l) ? 256 * hypot((t / 256) ...) : isnormal(l) ? sqrt(l) : 256 * hypot((t * 256) ...);
	}
	template <Arithmetic _T> inline constexpr Promoted<_T> hypot(_T t = 0) noexcept {
		return abs(t);
	}
	// 2.4 Trigonometric functions
	template <Arithmetic _T> inline constexpr Promoted<_T> sin(_T x) noexcept {
		if (x < 0) return -sin(-x);
	...
	template <Arithmetic _T> inline constexpr Promoted<_T> cos(_T x) noexcept {
		if (x < 0) return cos(-x);
	...
	template <Arithmetic _T> inline constexpr Promoted<_T> atan(_T x) noexcept {
		return asin(isinf(x) ? sign(x) : x / sqrt(x * x + 1.0L));
	}
	...
	// Supplementaries
	template <Arithmetic _T, Arithmetic _U> inline constexpr Promoted<_T, _U> atan2(_T y, _U x) noexcept {
		return (signbit(x) ? signbit(y) ? -M_PI : M_PI : 0) + atan(y * 1.0l / x);
	}
	template <Arithmetic _T> inline constexpr Promoted<_T> arg(Complex<_T> z) noexcept {
		atan2(z.imag, z.real);
	}
	...
	inline constexpr uintmax_t fibonacci(uintmax_t n) {
		...
		while (x < n) ++x & 1 ? (a += b) : (b += a);
		return a + b;
	}
	// 3 Factor operations
	template <Integral _T, Integral _U> inline constexpr auto gcd(_T x, _U y) -> decltype(x + y) {
		return x && y ? gcd(y, x % y) : abs(x + y);
	}
	template <Integral _T, Integral _U> inline constexpr auto lcm(_T x, _U y) -> decltype(x + y) {
	...
	template <Integral _T, Integral _U> inline constexpr CommonType<_T, _U> nPr(_T n, _U r) noexcept {
		return r ? nPr(n, r - 1) * (n - r + 1) : 1;
	}
	template <Integral _T, Integral _U> inline constexpr CommonType<_T, _U> nCr(_T n, _U r) noexcept {
		if (n < 0 || r < 0 || n < r) return 0;
		else if (r == 0 || r == n) return 1;
		else if (r > n / 2) r = n - r;
		uintmax_t* temp = new uintmax_t[r + 1]{1}, res = 0;
		for (_T t = 1; t <= n; ++t) for (_U u = t < r ? t : r; u; --u) temp[u] += temp[u - 1];
		res = temp[r];
		return delete[] temp, res;
	}
	// 4 Complex functions vvv
	template <Arithmetic _T> inline constexpr Unsigned<_T> abs(Complex<_T> z) noexcept {
		return hypot(z.real, z.imag);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> exp(Complex<_T> z) noexcept {
		return exp(z.real) * Complex<_T>{cos(z.imag), sin(z.imag)};
	}
	template <Arithmetic _T> inline constexpr Complex<_T> exp2(Complex<_T> z) noexcept {
		return exp(z * M_LN2);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> exp10(Complex<_T> z) noexcept {
		return exp(z * M_LN10);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> log(Complex<_T> z) noexcept {
		return {log(abs(z)), arg(z)};
	}
	template <Arithmetic _T> inline constexpr Complex<_T> log2(Complex<_T> z) noexcept {
		return M_LOG2E * log(z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> log10(Complex<_T> z) noexcept {
		return M_LOG10E * log(z);
	}
	template <Arithmetic _T, Arithmetic _U = float> inline constexpr Complex<_T> logn(Complex<_T> z, _U y) noexcept {
		return log(z) / log(y);
	}
	...
	template <Arithmetic _T> inline constexpr Complex<_T> cbrt(Complex<_T> z) noexcept {
		return cbrt(z.real) * Complex<_T>{cos(z.imag / 3), sin(z.imag / 3)};
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> sin(const Complex<_T>& z) noexcept {
		return {sin(z.real) * cosh(z.imag), cos(z.real) * sinh(z.imag)};
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> cos(const Complex<_T>& z) noexcept {
		return {cos(z.real) * cosh(z.imag), -sin(z.real) * sinh(z.imag)};
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> tan(const Complex<_T>& z) noexcept {
		return sin(z) / cos(z);
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> cot(const Complex<_T>& z) noexcept {
		return cos(z) / sin(z);
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> sec(const Complex<_T>& z) noexcept {
		return 1 / cos(z);
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> csc(const Complex<_T>& z) noexcept {
		return 1 / sin(z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> asin(Complex<_T> z) noexcept {
		return 1il * log(sqrt(1 - z * z) - 1i * z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> acos(Complex<_T> z) noexcept {
		return 1il * log(z - 1i * sqrt(1 - z - z));
	}
	template <Arithmetic _T> inline constexpr Complex<_T> atan(Complex<_T> z) noexcept {
		return 0.5il * (log(1i + z) - log(1i - z));
	}
	template <Arithmetic _T> inline constexpr Complex<_T> acot(Complex<_T> z) noexcept {
		return 0.5il * (log(z - 1i) - log(z + 1i));
	}
	template <Arithmetic _T> inline constexpr Complex<_T> asec(Complex<_T> z) noexcept {
		return acos(1.0L / z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> acsc(Complex<_T> z) noexcept {
		return asin(1.0L / z);
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> sinh(const Complex<_T>& z) noexcept {
		return {sinh(z.real) * cos(z.imag), cosh(z.real) * sin(z.imag)};
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> cosh(const Complex<_T>& z) noexcept {
		return {cosh(z.real) * cos(z.imag), sinh(z.real) * sin(z.imag)};
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> tanh(const Complex<_T>& z) noexcept {
		return sinh(z) / cosh(z);
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> coth(const Complex<_T>& z) noexcept {
		return cosh(z) / sinh(z);
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> sech(const Complex<_T>& z) noexcept {
		return 1 / cosh(z);
	}
	template <FloatingPoint _T> inline constexpr Complex<_T> csch(const Complex<_T>& z) noexcept {
		return 1 / sinh(z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> asinh(Complex<_T> z) noexcept {
		return log(sqrt(1 + z * z) + z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> acosh(Complex<_T> z) noexcept {
		return log(sqrt(z * z - 1) + z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> atanh(Complex<_T> z) noexcept {
		return log((1 + z) / (1 - z)) * 0.5l;
	}
	template <Arithmetic _T> inline constexpr Complex<_T> acoth(Complex<_T> z) noexcept {
		return log((z + 1) / (z - 1)) * 0.5l;
	}
	template <Arithmetic _T> inline constexpr Complex<_T> asech(Complex<_T> z) noexcept {
		return acosh(1.0L / z);
	}
	template <Arithmetic _T> inline constexpr Complex<_T> acsch(Complex<_T> z) noexcept {
		return asinh(1.0L / z);
	}
	// 7 Special functions
	// 7.1 Error functions
	...
	template <Arithmetic _T> inline constexpr Promoted<_T> tgamma(_T x) noexcept {
		if (isnan(x) || x == 0) return nan(errno = EDOM);
		else if (x < 0) return tgamma(x + 1) / x;
		else if (x > limits<Promoted<_T>>::max_exponent) return infinity();
		else if (x == 1 || x == 2) return 1;
		else return exp(lgamma(x));
	}
	// 7.3 Beta function
	...
	template <Arithmetic _T> inline constexpr Promoted<_T> riemannZeta(_T x) noexcept {
		if (isnan(x)) return nan(errno = EDOM);
	...
	// 8 Bessel functions
	// 8.1 Cylindrical
	template <Arithmetic _T> inline constexpr Promoted<_T> cyllindricalBesselI(_T x) {

	}
	template <Arithmetic _T> inline constexpr Promoted<_T> cyllindricalBesselJ(_T x) {

	}
	template <Arithmetic _T> inline constexpr Promoted<_T> cyllindricalBesselK(_T x) {
	...
	// Alternative names
	auto ln(auto a) noexcept {
		return log(a);
	}
	auto lg(auto a) noexcept {
		return log10(a);
	}
	auto lb(auto a) noexcept {
		return log2(a);
	}
}
#else
